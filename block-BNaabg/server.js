...
//       const filepath = userDir + username + ".json";
...
//         res.statusCode = 500; // Set appropriate status code for the error
...
function getUserFileInfo(store) {
  const requestData = querystring.parse(store);
  const username = requestData.username;
...
function handlePostRequest(req, res, store) {
    const { requestData, username, filepath } = getUserFileInfo(store);
...
function handleRequest(req, res) {
  ...
  let store = ""; // Define the store variable here
